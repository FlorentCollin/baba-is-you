...
public class Board {
	...
	/**
	 * Méthode qui va renvoyer l'ensemble des cellules changées et qui va remettre é 0 cette liste
	 ...
	 * Cette méthode va étre trés utile pour optimiser la vitesse du programme
	 ...
	 * @param endItem Ce en quoi baseItem doit étre changé
	 */
	...
	public boolean isWin()
	{
		...
		//Optimisation de l'algo pour éviter d'itérer sur l'entiéreté des joueurs si il n'y a pas d'item gagnant
		if(wordInRule == null)
		{
			return false;
		}
		for(Tuple player : players)
		{
			...
			{
				if(element.isRepresentedBy(wordInRule)) //Regarde si l'item correspond é l'Item permettant de réussir le niveau
				{
					return true;
				}
			}
	...
	/**
	 * Méthode qui va scanner la map et rechercher les différents joueurs en fonction des régles actives
	 *
	 */
	public void searchPlayers()
	{
		...
		//Optimisation de l'algo qui évite de parcourir l'entiéreté de la map si il n'y a aucun joueur
		if(playerIs.size() == 0)
		{
			return;
		}
		...
		for(int y = 0; y < cols; y++)
		{
			for(int x = 0; x < rows; x++)
			{
				...
				//Le z correspond é l'index dans la liste d'Item de la cellule et ici on itére sur la liste d'Item
	...
	{
		//Note : on renvoit une ArrayList<IRule> car il est possible d'avoir plusieurs sortes de joueurs en méme temps
	 ...
	 * @param x1 position en x de l'item é changer de cellule
	 * @param y1 position en y de l'item é changer de cellule
