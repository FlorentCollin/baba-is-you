...
public class LevelsChoice extends Menu {
	...
	private static void initButton() {
		...
		loadImage = new ImageView(new Image("file:code" + File.separator + "ressources" + File.separator + "load.png"));
		...
		loadImage.setOnMouseClicked((MouseEvent e) -> {
			...
			Level.loadLevel(fileChoose.getPath().split(Pattern.quote("."))[0], true, true); // le Pattern.quote permet
																							// de supprimer le .txt
		});
	...
	public static void loadLevelsMenu() {
		...
		gc.drawImage(new Image("file:code" + File.separator + "ressources" + File.separator + "levelsMenu.png"), 0, 0); // Affichage de l'image de
																								// fond

		// Boucle permettant d'afficher et de contrôler les différents bouttons de
		// niveaux (ex : LVL 1, LVL 2,...)
		int x = -65;
		for (int index = 0; index <= LevelManager.getListOfLevels().length; index++) {
			ImageView levelImage = new ImageView(
					new Image("file:code" + File.separator + "ressources" + File.separator + "level" + index + ".png"));
			levelImage.setPickOnBounds(true); // permet de cliquer sur l'entireté de l'image et non pas juste sur les
												// pixels visibles
		...
		root.getChildren().addAll(loadImage, close); // Ajout des bouttons close et loadImage définient dans
														// initButton()
