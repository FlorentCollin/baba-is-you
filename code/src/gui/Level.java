...
public class Level extends BabaIsYouApp {
	 ...
	 *            le niveau à la suite de ceux-ci
	 * @param activateInputs
	 *            Est ce que les "Inputs" du clavier peuvent être utilisé
	 */
	public static void loadLevel(String name, boolean eraseActivesBoards, boolean activateInputs) {
		...
		CELL_SIZE = canvas.getHeight() / Math.max(board.getCols(), board.getRows()); // De combien doit être la taille
																						// d'une cellule
		/*
		 * Ce StackPane est représente le fond foncé qu'il y a sur les différents
		 * niveaux Cela permet de ne pas devoir afficher un fond de cette couleur à
		...
		for (int i=0; i<array[0].length; i++) {
			for (int j=0; j<array.length; j++) {
				...
				for (Item element3 : array[j][i].getList()) {
					...
					imageItem = new Image("file:" + ressourcesPath + element3.getName() + ".png",
	...
	protected static void drawCell(int x, int y) {
		...
		for (Item element1 : board.getCell(x, y).getList()) {
			// Récupération de l'image à afficher en fonction de son nom
			imageItem = new Image("file:" + ressourcesPath + element1.getName() + ".png", CELL_SIZE,
					CELL_SIZE, true, true);
			// On dessine l'Item en fonction de la taille d'une cellule
		...
		game.setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				case "left":
					direction = 3;
					break;
				case "save": // Sauvegarde le niveau en cours
					LevelManager.saveLvl(levelsPath + "saves" + File.separator + "save");
					return;
