...
public class BabaIsYouApp extends Application {
	...
	public void start(Stage Stage) throws Exception {
		...
		primaryStage.getIcons().add(new Image("file:"+ ressourcesPath + File.separator + "Baba.png"));
		...
		fontMadness = Font.loadFont(
				"file:" + ressourcesPath + File.separator + "fonts" + File.separator + "8-Bit Madness.ttf", 21);
		fontMadnessBig = Font.loadFont(
				"file:" + ressourcesPath + File.separator + "fonts" + File.separator + "8-Bit Madness.ttf", 30);
		if (fontMadness == null)
	...
	/**
	 * Méthode qui va initialiser ce que l'application à besoin pour fonctionner
	...
	public void init() {
		...
		success = new Success();
		sound = new SoundFX();
		SoundFX.setSound(settings.getSoundSettings().get("SOUNDFX"));
	}
	...
	protected static File FileChooserLvl() {
		...
		fc.setInitialDirectory(new File("code" + File.separator + "levels" + File.separator + "editor")); // On définit l'endroit où le
	...
	public static void showSuccessUnlocked() {
		ImageView gif = new ImageView(new Image("file:" + ressourcesPath+File.separator+"SuccessUnlockedAnimation.gif"));
		gif.setX(750);
		gif.setY(30);
